...
For this example, we will select all voxels that meet 3 criteria:
...
We will then use SPM to extract timeseries from these voxels and pre-process them (removing the mean and motion confounds, high-pass filtering and pre-whitening).

***2a. Specify a GLM and add a contrast***

We'll use a GLM to identify voxels responding to the stimuli. This initial feature selection step also means that we won't waste time estimating Bayesian models on voxels which do not respond to visual stimuli. Use SPM to specify and estimate a first level (GLM) analysis with one or more task conditions (leaving baseline periods unmodelled). Also include motion regressors for each session and any other physiological regressors you may have available. Add a single F-contrast called 'Effects of Interest', with the contrast matrix: eye(N), where N is the number of task-relevant regressors in a single session.

***2b. Make a mask of all responsive voxels at p < 0.001***
View the results of the Effects of Interest contrast, thresholded (for example) at p < 0.001 uncorrected. Then in the SPM results window, select Save -> Binary mask. Call this mask_uncorrected.

***2c. Refine the mask to only include voxels from the back of the brain***
We can edit the mask we just created (with filename 'GLM/spmF_0001_mask_uncorrected.nii') to only include voxels towards the back of the brain. Here's a Matlab script, which needs to be run from within the subject's GLM direcory:
...
***2d. Make a mask of the cortical surface***
Now we'll make masks of all voxels on the cortical surface (one for the left and one for the right hemisphere). The Freesurfer surface extraction has already been performed for you (using the recon-all command). Run the following Matlab code to import the Freesurfer surfaces:
...
Where data_dir is the directory containing the T1 structural (the pRF subdirectory of the downloaded data) and surf_dir is the directory containing the files generated by Freesurfer (the surf sub-directory of the downloaded data). You may wish to check that nifti images created here (GLM/lh_surface.nii and GLM/rh_surface.nii) align with your T1 image and SPM analysis.

*** 2e. Extract the timeseries***
We'll now extract timeseries from all the voxels within our posterior mask ('GLM/spmF_0001_mask_uncorrected.nii') which are also within our cortical surface masks (GLM/lh_surface.nii and GLM/rh_surface.nii).

In the main SPM window, select Batch. Then select SPM -> Util -> Volume of interest from the menu at the top. Fill in the boxes as follows:

- Select SPM.mat - Select your subject's SPM.mat
- Adjust Data - select the index of the Effects of Interest contrast you created above. If you only added one contrast, enter: 1.
- Which sesison - type: Inf . This will extract timeseries for all sessions (runs). 
- Name of VOI - enter a name, we'll just call this Mask.
- Region(s) of Interest - Click New: Mask Image twice. For the first 'Image file' entry, select the mask of the back of the brain (GLM/spmF_0001_mask_uncorrected.nii). For the second 'Image file' entry, select either (GLM/lh_surface.nii and GLM/rh_surface.nii) for the left or right hemisphere respectively.
- Expression - type (i1 & i2) to include voxels which appear in mask 1 AND mask 2.

Then click the green play button. For each run this will create a file named VOI_xx.mat, containing a summary timeseries, as well as timeseries for every voxel in the ROI. 

*** 2f. Collate the filenames of the extracted timeseries ***
Create a cell array containing the filenames of each session's VOI:
...
We'll now give this xY cell array to the BayesPrf toolbox. 
